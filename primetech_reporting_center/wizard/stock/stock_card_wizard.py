...
class StockCardWizard(models.TransientModel):
    ...
    _description = 'PrimeTech Stock Card Wizard'
    ...
    def action_preview(self):
        ...
        html = self.env['ir.qweb']._render('primetech_reporting_center.stock_card_preview_template', {'data': report_data, 'filters': filters, 'wizard': self, 'generated_at': datetime.now().strftime('%d/%m/%Y %H:%M')})
        preview = self.env['primetech.purchase.preview.wizard'].create({'name': 'Fiche de Stock', 'html': html, 'pdf_report_ref': 'primetech_reporting_center.action_stock_card_pdf', 'xlsx_report_ref': 'primetech_reporting_center.action_stock_card_xlsx', 'wizard_id': self.id, 'wizard_model': 'pt.stock.card.wizard'})
    ...
    def action_print_pdf(self):
        self.ensure_one()
        return self.env.ref('primetech_reporting_center.action_stock_card_pdf').report_action(self, data={'filters': self._prepare_filters()})

    def action_export_xlsx(self):
        self.ensure_one()
        return self.env.ref('primetech_reporting_center.action_stock_card_xlsx').report_action(self, data={'filters': self._prepare_filters()})
